	...
	try {
		...
		repository.Find(card.card_id, card.email, card.document_number, function(c) {
			if (!c) {
				...
				transactionRepository.Create(c.card_id, "loyaltycard", orderNumber, value, function(transactionId) {
					var responseCard = {
...
function GetHtml(card, callback) {
	...
	transactionRepository.GetAllByCard(card.card_id, function(transactions) {
		...
		if (transactions != null) {
			html += "<table class=\"rf-default-table table\"><thead><tr><th>Código da transação</th><th>Data</th><th>Valor</th></tr></thead><tbody>";
			...
			if(transactions.length == 0){
				html += '<tr><td colspan="3" align="center"> Nenhuma transação encontrada. </td></tr>';
			}
